...
public interface IPurchasedProductRepository extends JpaRepository<PurchasedProduct, PurchasedProduct> {
    ...
    @Query("SELECT pp FROM PurchasedProduct pp where pp.userEntity.id = :userEntityId " +
            "AND pp.product.id = :productId AND pp.cartStatus = 'IN_PROGRESS'")
    PurchasedProduct findPurchasedProduct(@Param("userEntityId") Long userEntityId,
                                          @Param("productId") Long productId);

    @Query("SELECT pp FROM PurchasedProduct pp WHERE pp.userEntity.id = :idCustomer AND pp.cartStatus = 'IN_PROGRESS'")
    ...
    @Query("SELECT new  gestionInventario.com.model.dto.purchasedProduct.PurchasedProductDTO" +
            ...
            "WHERE pp.userEntity.id = :idCustomer AND pp.cartStatus = :cartStatus")
    ...
    @Modifying
    @Query("UPDATE PurchasedProduct pp SET pp.cartStatus = 'CANCELED' WHERE pp.cartStatus = 'IN_PROGRESS' AND pp.product.id = :idProduct AND pp.userEntity.id = :idCustomer")
